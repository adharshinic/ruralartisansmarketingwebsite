...
<?php
							 $sql = "SELECT * FROM `purchase_request` INNER JOIN product ON product.product_id=purchase_request.product_id  WHERE product.artist_id='$_SESSION[artistid]'";
...
else
{
	 if($rs[7] == "Pending")
	{
	echo "<a href='viewpurchaseorderbill.php?purchaserequestid=$rs[0]' class='btn btn-primary' style='width: 100%;'>Send Quotation</a>";
	}
	else
	{
...
{
echo "<hr><a href='viewartistpurchaserequest.php?deleteid=$rs[0]' onclick='return delconfirm()' class='btn btn-danger' style='width: 100%;'>Delete</a>";
}				 							 
echo "</td></tr>";
}
							  ?>
