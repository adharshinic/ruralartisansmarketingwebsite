...
if($_SESSION['randnumber']  == $_POST['randnumber'])
{
	...
	{	
		...
		else
		{
			...
			{
				$sql ="UPDATE worker_request SET artist_id='$_SESSION[artistid]', from_date='$_POST[fromdate]', to_date='$_POST[tilldate]', task='$_POST[task]', country_id='$_POST[country]', state_id='$_POST[state]', city_id='$_POST[city]', salary='$_POST[salary]', salary_type='$_POST[salarytype]', artist_status='Pending', worker_status='', artist_comment='$_POST[retruitercomment]', worker_comment='' WHERE worker_request_id='$_GET[editid]'";
			...
			{
				$sql = "INSERT INTO worker_request( worker_id,artist_id, from_date, to_date, task, country_id, state_id, city_id, salary, salary_type, artist_status, worker_status, artist_comment, worker_comment) VALUES ('$_POST[workerid]','$_SESSION[artistid]','$_POST[fromdate]','$_POST[tilldate]','$_POST[task]','$_POST[country]','$_POST[state]','$_POST[city]','$_POST[salary]','$_POST[salarytype]','Approved','','$_POST[retruitercomment]','')";
				if(!mysqli_query($con,$sql))
				{
				...
				else
				{
					...
					$sqlartist = "SELECT * FROM artist WHERE artist_id='$_SESSION[artistid]'";
					...
					$msgtoworker = "You have got work request from " . $rsartist['artist_name'] . " for " . $_POST['task'] . " from ". $_POST['fromdate'] . " to " .  $_POST['tilldate'] . ". Kindly login the website to approve/reject the request..";
					sendmail($rsworker['login_id'], $rsworker['name'] , "ZimCrafts - Work Request Notification", $msgtoworker);
					//###Mail Code Ends here
					//###################################################
					//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
					//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
					...
					if($smsstatus == "Enabled")
					{
						...
						$msg= "You have got work request from " . $rsartist['artist_name'] . " for " . $_POST['task'] . " from ". $_POST['fromdate'] . " to " .  $_POST['tilldate'] . ". Kindly login the website to approve/reject the request..";
						send_sms($mobno,$msg);
					}
					//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
					//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
{
		$sqlartist = "SELECT * FROM artist WHERE artist_id='$rsworkq[artist_id]'";
		$qsqlartist = mysqli_query($con,$sqlartist);
		$rsartist = mysqli_fetch_array($qsqlartist);										  						  
...
<div class="form-row">
	...
	Any Comments or Note <font color="#FF0000">*</font>
	  <textarea name="retruitercomment" id="retruitercomment" class="form-control"><?php echo $rsedit['artist_comment']; ?></textarea>
	</div>	
	
</div>

<hr>
<button type="submit" name="submit" id="submit" class="btn btn-danger btn-lg btn-block" >Submit</button>
