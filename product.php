...
if(isset($_POST['submit']))
{
	...
	{
		$qty = $_POST['quantity'];
		$sql ="UPDATE product SET category_id='$_POST[category]', produce_id='$_POST[produce]', variety_id='$_POST[variety]', title='". mysqli_real_escape_string($con,$_POST['title']) ."', ";
		if($_FILES['img1']['name'] != "")
		{
		...
		$sql = $sql . " quantity='$_POST[quantity]', quantity_type='$_POST[quantitytype]', description='" . mysqli_real_escape_string($con,$_POST['description'])  . "', uploaded_date='$dt', status='$status' WHERE product_id='$_GET[editid]'";
		...
		{
			echo "<script>alert('Artwork Record updated successfully...');</script>";
			echo "<script>window.location='viewProduct.php';</script>";
		}
	}
	else
	{
		$sql="INSERT INTO product(product_id, artist_id, category_id, produce_id, variety_id, title, img_1";
		if($_FILES['img2']['name'] != "")
		{
		$sql = $sql . " , img_2";
		}
		...
		$sql = $sql . " , quantity, quantity_type, description, uploaded_date, status) VALUES ('','$_SESSION[artistid]','$_POST[category]','$_POST[produce]','$_POST[variety]','". mysqli_real_escape_string($con,$_POST['title']) ."','$imgname1'";
		if($_FILES['img2']['name'] != "")
		{
		$sql = $sql . " ,'$imgname2'";
		}
		...
		$sql = $sql . " ,'$_POST[quantity]','$_POST[quantitytype]','". mysqli_real_escape_string($con,$_POST['description']) ."','$dt','$status')";
		if(!mysqli_query($con,$sql))
		{
			echo "Error in mysqli query";
		}
		else
		{
			...
			{
			echo "<script>alert('Your Artwork Has Been Put On Sale...');</script>";
			}
			else
			{
			echo "<script>alert('Your Artwork has not been put on sale...');</script>";
			}
			echo "<script>window.location='viewProduct.php';</script>";
		}
	}
  ...
  <main id="main">
    ...
    <section id="contact" class="contact">
      <div class="container">
		...
		<center><h4>Enter Craft Detail...</h4></center><hr>
...
<div class="form-row">
	...
	<div class="col-md-4 form-group">
	Craft Type <font color="#FF0000">*</font>
                                <div id="txtHint">
...
function loadstatus(prodstatus)
{
	...
	{
	document.getElementById("ajaxstatus").innerHTML = "Your artwork will be put on sale..";
	}
	else
	{
	document.getElementById("ajaxstatus").innerHTML = "Your Artwork will not be put on sale because the status is inactive. Put it on Active to display the products";
	}
}
</script>