<?php 
include("header.php");
if(!isset($_SESSION['customerid']) && !isset($_SESSION['artistid']))
{
...
if(isset($_GET['prodid']))
{
	...
	{
		$sql = "DELETE FROM product_purchase_record WHERE selling_prod_id='$_GET[prodid]' AND status='Pending' AND artist_id='$_SESSION[artistid]'";
		$qsql = mysqli_query($con,$sql);
	}
	if(isset($_SESSION['customerid'])){
		$sql = "INSERT INTO product_purchase_record(product_purchase_bill_id, selling_prod_id,customer_id, quantity, cost, status) VALUES ('0','$_GET[prodid]','$_SESSION[customerid]','1','$_GET[prodcost]','Pending')";
		$qsql = mysqli_query($con,$sql);
		echo "<script>alert('Product added to the cart');</script>";
		echo "<script>window.location='displaycart.php';</script>";
	}
	if(isset($_SESSION['artistid'])){
		$sql = "INSERT INTO product_purchase_record(product_purchase_bill_id, selling_prod_id, quantity, cost, status,artist_id) VALUES ('0','$_GET[prodid]','1','$_GET[prodcost]','Pending','$_SESSION[artistid]')";
		$qsql = mysqli_query($con,$sql);
		echo "<script>alert('Product added to the cart');</script>";
		echo "<script>window.location='displaycart.php';</script>";
	}
...
<hr>
<center>
<input type="submit" name="submit" id="submit" value="Confirm your order" autofocus class="btn btn-danger">
</center>
            </div>
		  </div>
