...
if($_SESSION['randnumber']  == $_POST['randnumber'])
{
	if(isset($_POST['submit']))
	{
		...
		{
			$sql ="UPDATE produce SET  `category_id`='$_POST[category]', `produce`='$_POST[produce]', `description`='$_POST[description]'";
			if($_FILES['img']['name'] != "")
			{
			$sql = $sql . ", `img`='$imgname1'";
			}
			$sql = $sql . ", `status`='$_POST[status]'  WHERE produce_id='$_GET[editid]'";
  ...
  <main id="main">
    ...
    <section id="contact" class="contact">
      <div class="container">
		...
		<center><h4>Enter Craft Type Detail...</h4></center><hr>
...
<div class="form-row">
	...
	<div class="col-md-4 form-group">
	  ...
	  <?php
		if(isset($_GET['editid'])){
			if(file_exists("imgproduce/".$rsedit['img']))
			{
				echo "<img src='imgproduce/".$rsedit['img'] ."' style='height: 150px;'>";
...
?>
	...
	function validateproduce()
	{
		 if(document.frmproduce.category.value == "")
			...
			else if(document.frmproduce.img.value == "" && document.frmproduce.imgset.value == 0)
			{
