...
if(isset($_POST['submit']))
{
	...
	{
		$sql ="UPDATE product SET  artist_id='$_SESSION[artistid]', category_id='$_POST[category]', produce_id='$_POST[produce]', variety_id='$_POST[variety]', title='$_POST[title]', img_1='$imgname1', img_2='$imgname2', img_3='$imgname3', img_4='$imgname4', img_5='$imgname5', quantity='$_POST[quantity]', quantity_type='$_POST[quantitytype]', description='$_POST[description]', uploaded_date='$_POST[uploaddate]', status='$_POST[status]' WHERE product_id='$_GET[editid]'";
	...
	{
$sql="INSERT INTO product(artist_id, category_id, produce_id, variety_id, title, img_1, img_2, img_3, img_4, img_5, quantity, quantity_type, description, uploaded_date, status) VALUES (";
if(isset($_SESSION['artistid']))
{
	$sql = $sql ." '$_SESSION[artistid]'";
}
else
{
	$sql = $sql . "'" .$_POST[artistid] . "'";
}
  ...
  <main id="main">
    ...
    <section id="contact" class="contact">
      <div class="container">
		...
		<center><h4>Enter Artworks Detail...</h4></center><hr>
...
<input type="hidden" name="randnumber" value="<?php echo $randnumber; ?>" >
	...
	artist Name <font color="#FF0000">*</font>
	  <select name="artistid" id="artistid" autofocus class="form-control">
			<option value="">Select Status</option>
			<?php
			  $sql2 = "SELECT * FROM artist where status='Active'";
				$qsql2 =mysqli_query($con,$sql2);
			  while($rssql2 = mysqli_fetch_array($qsql2))
			  {
				  if($rssql2['artist_id'] == $rsedit['artist_id'] )
				  {
				  echo "<option value='$rssql2[artist_id]' selected>$rssql2[artist_name] ( $rssql2[email_id] )</option>";
				  }
				  else
				  {
				  echo "<option value='$rssql2[artist_id]'>$rssql2[artist_name] ( $rssql2[email_id] )</option>";
				  }
			  }
			  ?>
		</select>
...
var emailExp = /^[\w\-\.\+]+\@[a-zA-Z0-9\.\-]+\.[a-zA-z0-9]{2,4}$/; //Variable to validate Email ID
	...
	if(document.frmsellingproduce.artistid.value == "")
	{
		alert("Kindly select a artist name..");
		document.frmsellingproduce.artistid.focus();
		return false;
	}
