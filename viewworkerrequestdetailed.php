...
if(isset($_POST['submit']))
{
	...
	{
		$sql = "UPDATE worker_request SET artist_status='$_POST[artiststatus]'";
		if($_POST['artistcomment'] != "")
		{
			$sql = $sql . ", artist_comment= '". date("d-M-Y h:i:s") ." - $_POST[artistcomment] \n $_POST[artistallcomment]'";
		}
	...
	if(isset($_SESSION['workerid']))
	{
				...
				$sqlartist = "SELECT * FROM artist WHERE artist_id='$rsworker_request[artist_id]'";
				...
				$msgtoartist = "Your request " . $_POST['workerstatus'] . " by $rsworker[name] for $rsworker_request[task] from $rsworker_request[from_date] to  $rsworker_request[to_date] ..";
				sendmail($rsartist['email_id'], $rsartist['artist_name'] , "ZimCrafts - Work Request " . $_POST['workerstatus'], $msgtoartist);
				//###Mail Code Ends here
				//###################################################
				...
				//while($rslworkerrequest = mysqli_fetch_array($qsqlworkerrequest))
				{
					...
					if($smsstatus == "Enabled")
					{
						...
						$msg= "Your request " . $_POST['workerstatus'] . " by $rsworker[name] for $rsworker_request[task] from $rsworker_request[from_date] to  $rsworker_request[to_date] ..";
						send_sms($mobno,$msg);
					}
					//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
					//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
						    ...
						    <tbody>
						      ...
						      <tr>
								...
								$sqlscntry= "SELECT * FROM country WHERE country_id='$rsworker[country_id]'";
								 ...
								 echo ucfirst($rsartist['artist_name']) ."<br>".
								      $rsartist['artist_address'] ."<br>".
								      $rsscty['city'] ."<br>".
								      $rssst['state'] ."<br>".
								      $rsscntry['country'] ."<br>".
								     "PIN-". $rsartist['pincode']; ?></th>
					          </tr> 
						      ...
						      <tr>
						        ...
						        <td><?php echo $usdsymbol; ?>&nbsp;<?php echo  $rs['salary'] ."&nbsp;(".  $rs['salary_type'] .")"; ?></td>
					          </tr>
						      <tr>
						        <th scope="row" style="vertical-align: top;"><p><strong>artist Comment:</strong></p>
								...
								if(isset($_SESSION['workerid']))
								{
									...
									{
										echo $rs['artist_status'];
									}
								}
								else
								{
								?>
                                  <input type="text" name="artiststatus" readonly value="Approved" >
                                <?php
								}
								?>
					            </p></th>
								...
								{
								?>
									                                <textarea name="artistallcomment" readonly style="width:250px;height:160px; background-color:rgba(239,235,235,1.00);" ><?php echo $rs['artist_comment']; ?></textarea><br>
                                <input type="text" name="artistcomment" style="width:250px;" placeholder="Post comment here" >
								<?php
                                }
								else
								{
									echo $rs['artist_comment'];
								}
								?>
                                </td>
					          </tr>
							  ...
							  ?>
						      <tr>
						        <th colspan="2" scope="row"><center><input type="submit" name="submit" id="submit" value="Update" class="btn btn-danger" ></center></th>
					          </tr>
                              <?php
							  }
							  ?>
