...
if(isset($_POST['submit']))
{
	...
	{	
		...
		else
		{
			...
			$sqlartist = "SELECT * FROM artist WHERE artist_id='$rsproduct[artist_id]'";
			...
			$sql1country = "SELECT * FROM country WHERE country_id='$rsartist[country_id]'";
			$qsql1country = mysqli_query($con,$sql1country);
			$rs1country = mysqli_fetch_array($qsql1country);
			$sql2state = "SELECT * FROM state WHERE state_id='$rsartist[state_id]'";
			$qsql2state = mysqli_query($con,$sql2state);
			$rs12state = mysqli_fetch_array($qsql2state);
			$sql3city = "SELECT * FROM city WHERE city_id='$rsartist[city_id]'";
			$qsql3city = mysqli_query($con,$sql3city);
			$rs13city = mysqli_fetch_array($qsql3city);
			$msgtoartist = "You have got purchase request for your Artwork - $rsproduct[title]. You can contact your customer $rscustomer[customer_name] at $rscustomer[mobile_no].";
			$msgtocustomer = "You have sent purchase request for - $rsproduct[title]. To check the quality of the Artwork, you can contact $rsartist[artist_name] at $rsartist[mobile_no]. Farmer's Address: $rsartist[artist_address], $rs13city[city], $rs12state[state], $rs1country[country].  ";
				//###Mail Code Starts here
				include("phpmailer.php");
				sendmail($rsartist['email_id'], $rsartist['artist_name'] , "ZimCrafts - Purchase Request from customer", $msgtoartist);
				sendmail($rscustomer['email_id'], $rscustomer['customer_name'] , "ZimCrafts - Purchase Request sent to artist", $msgtocustomer);
				//###Mail Code Ends here
				//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
				//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
				include("smsapi.php");
				...
				//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
					...
					sleep(5);
					echo "<script>window.location='viewpurchaserequest.php';</script>";
	}
}
...
?>	
          ...
          <div class="portfolio-info">
            ...
            <ul>
              ...
              <li><strong>Artist name:</strong>: <?php echo $rsartist['artist_name']; ?></li>
              <li><strong>Category</strong>: <?php echo $rscategory['category']; ?></li>
              <li><strong>Craft Type</strong>: <?php echo $rsproduce['produce']; ?></li>
...
<h4>Send A Purchase Request</h4>
  ...
  <?php
				if(isset($_POST['submit']))
				{
					echo "<h2>Purchase request sent successfully..</h2><h3><a href='viewpurchaserequest.php' class='btn btn-danger'>View purchase request</a></h3>";
				}
				else
				{
...
<textarea name="note" id="note" class="form-control" ><?php echo $rsedit['note']; ?></textarea>
	...
	<div class="col-md-12 form-group">
	<input type="submit" name="submit" id="submit" value="Click Here to send purchase request.." class="btn btn-danger">
	</div>
						   ...
						   {
							?>
                             <h2><a href='customerloginpanel.php?pagename=<?php echo basename($_SERVER['PHP_SELF']); ?>&productid=<?php echo $rs['product_id']; ?>'  class="btn btn-danger">Login to send purchase request..</a></h2>  
                           <?php
						   }
				}
						   ?>
