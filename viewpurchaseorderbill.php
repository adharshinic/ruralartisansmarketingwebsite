...
if(isset($_POST['submit']))
{
	...
	{	
	  ...
	  if(!mysqli_query($con,$sqlupd)){
			...
			$msg = "Your order for $rsproduct[title] has been placed successfully with the bill amount  - US $". $_POST['price'] . ".. Kindly make the payment before $_POST[expdate].. Otherwise the order will be cancelled..";
			//###Mail Code Starts here
			include("phpmailer.php");
			sendmail($rscustomer['email_id'], $rscustomer['customer_name'] , "ZimCrafts - Quotation of your Purchase request updated", $msg);
			//###Mail Code Ends here
	...
	else
	{
		...
		$sqlins="INSERT INTO `purchase_order`(`product_id`, `purchase_request_id`, `customer_id`, `artist_id`, `purchase_order_date`, `purchase_order_time`, `purchase_amt`, `quantity`, `status`) VALUES ('$_POST[product_id]','$_POST[purchase_request_id]','$_POST[customer_id]','$_SESSION[artistid]','$_POST[request_date]','$_POST[request_time]','$_POST[price]','$_POST[quantity]','Pending')";
		...
		else
		{
			...
			//###Mail Code Starts here
			include("phpmailer.php");
			sendmail($rscustomer['email_id'], $rscustomer['customer_name'] , "ZimCrafts - Quotation sent for your Purchase request", $msg);
			//###Mail Code Ends here
			//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
			//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
			include("smsapi.php");
  ...
  <main id="main">
    ...
    <section id="contact" class="contact">
      <div class="container">
		...
		<center><h4>Kindly enter your budget for this item...</h4></center><hr>
							...
							<table class="table table-striped table-bordered"  style="width:100%">
						      ...
						      <th><strong>Status</strong></th>
								...
								<TBODY>
							<?php
							  $sql = "SELECT *,purchase_request.quantity as purchase_request_quantity,purchase_request.status as purchase_request_status FROM `purchase_request` INNER JOIN product ON product.product_id=purchase_request.product_id  WHERE product.artist_id='$_SESSION[artistid]' AND purchase_request.purchase_request_id='$_GET[purchaserequestid]'";
							  $qsql = mysqli_query($con,$sql);
						    ...
						    <tr>
						      ...
						      <td>&nbsp;" . date("d-m-Y",strtotime($rs['request_date'])) ."</td>
						      <td>&nbsp;" . date("d-m-Y",strtotime($rs['request_date_expire'])) ."</td>
						      <td>&nbsp;$rs[note]</td>
 ...
 <tbody>
        ...
        <tr>
		 ...
		 $rspro = mysqli_fetch_array($qsqlproduct);
		  ...
		  ?>)</td>
          <td><input type="text" name="qty" id="qty" value="<?php echo $rs['purchase_request_quantity']; ?>" readonly class="form-control"></td>
        </tr>
        <tr>
          <td>Total cost:&nbsp;</td>
        ...
        <tr>
          <th scope="row">&nbsp;</th>
          <td><input type="submit" name="submit" id="submit" value="Submit" class="btn btn-danger"></td>
        </tr>
      </tbody>
    </table>
</form>	
